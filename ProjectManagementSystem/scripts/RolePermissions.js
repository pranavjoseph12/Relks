...
var isIncomeAcccess = false;
var isQuotationAcccess = false;
var isReportsAcccess = false;
var isAddUserToProjViewAccess = false;
var isAddUserToProjEditAccess = false;
var isAddUserToProjDeleteAccess = false;
var isIncomeViewAccess = false;
var isIncomeEditAccess = false;
var isIncomeDeleteAccess = false;
var isQuotationViewAccess = false;
var isQuotationEditAccess = false;
var isQuotationDeleteAccess = false;
GetRolePermissions();
...
function SetRolePermissions(response) {
    ...
    $.each(jsonResponse, function (key, role) {
        ...
        else if (role.ActionName == 'Income') {
            isIncomeViewAccess = role.ViewAccess;
            isIncomeEditAccess = role.EditAccess;
            isIncomeDeleteAccess = role.DeleteAccess;
        }
        else if (role.ActionName == 'Quotation') {
            isQuotationViewAccess = role.ViewAccess;
            isQuotationEditAccess = role.EditAccess;
            isQuotationDeleteAccess = role.DeleteAccess;
        }
    });
    ...
    if (!(isExpenseViewAccess == 'True' || isExpenseEditAccess == 'True' || isExpenseDeleteAccess == 'True' || isIncomeViewAccess == 'True' || isIncomeEditAccess == 'True' || isIncomeDeleteAccess == 'True')) {
        $('#liExpense').css('display', 'none');
    }
    else {
        $('#liExpense').css('display', 'block');
        $('#liPersonal').css('display', 'none');
        $('#liViewExpense').css('display', 'none');
        $('#liAddIncome').css('display', 'none');
        $('#liViewIncome').css('display', 'none');
        isExpenseAcccess = true;
        if (!(isExpenseViewAccess == 'False' && isExpenseViewAccess == 'False' && isExpenseViewAccess == 'False')) {
            
            $('#liPersonal').css('display', 'block');
            $('#liViewExpense').css('display', 'block');
        }
        if (!(isIncomeViewAccess == 'False' && isIncomeEditAccess == 'False' && isIncomeDeleteAccess == 'False')) {
            isIncomeAcccess = true;
            $('#liAddIncome').css('display', 'block');
            $('#liViewIncome').css('display', 'block');
        }
    }
    ...
    if (isQuotationViewAccess == 'False' && isQuotationEditAccess == 'False' && isQuotationDeleteAccess == 'False') {
        $('#liQuotations').css('display', 'none');
    }
    else {
        $('#liQuotations').css('display', 'block');
        if (isQuotationViewAccess == "True") {
            
            $('#liViewQuotation').css('display', 'block');
           }
        else {
            $('#liViewQuotation').css('display', 'none');
            $('#divQuotViewContent').html('<div class="box-header with-border">You dont have the permission for this action</div>');

        }
        if (isQuotationEditAccess == "True")
            $('#liCreateQuotation').css('display', 'block');
        else
            $('#liCreateQuotation').css('display', 'none');
        isReportsAcccess = true;
    }
  //  $('#liQuotations').css('display', 'block');
