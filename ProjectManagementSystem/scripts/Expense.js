...
$(document).ready(function () {
    if (isExpenseEditAccess != "True")
        $("#liPersonal").css('display', 'none');
    if (isIncomeEditAccess != "True")
        $("#liAddIncome").css('display', 'none');
    if (isExpenseViewAccess != "True")
        $("#liViewExpense").css('display', 'none');
    if (isIncomeViewAccess != "True")
    ...
    $("#liPersonal ,#liViewExpense , #liAddIncome ,#liViewIncome").unbind().bind('click', function () { ResetToAddExpense(); });
...
function BindProjectExpenses(response) {
    ...
    for (var i = 0; i < jsonResponse.length; i++) {
        ...
        if(isExpenseEditAccess == "True")
             trObj += '<div class="btn btn-warning small_sp" style="margin-right: 5px;" onclick="return EditExpense(\' ' + jsonResponse[i].ProjectExpenseId + '\',\' ' + jsonResponse[i].ExpenseName + '\',\' ' + jsonResponse[i].Category + '\',\' ' + jsonResponse[i].ProjectName + '\',\' ' + jsonResponse[i].PhaseName + '\',\' ' + jsonResponse[i].Amount + '\',\' ' + jsonResponse[i].ExpenseDate + '\',\' ' + jsonResponse[i].Comments + '\');"><i class="fa fa-pencil"></i></div>';
        if(isExpenseDeleteAccess=="True")
